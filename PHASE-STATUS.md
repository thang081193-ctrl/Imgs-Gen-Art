...
Current phase: **Phase 6 — CLOSED. v1 READY.** Session #32 drained the ship-critical polish backlog in a single pass: F1 CF#27 parallel ENOTEMPTY race fix (forks singleFork pool + rmSync maxRetries:5 retryDelay:200 — 5-run clean) + F2 clean-sweep dead-code deletion (`/api/profiles/:id/export` + `/api/profiles/import` both proven dead via grep; handoff §F2 expanded from /export-only to Option B clean sweep) + F3 Gallery custom date picker (schema extension with `dateFrom`/`dateTo` ISO fields + SQL builder custom-range override with local-day boundaries + inline DateSection expander + chip resolver "2026-04-20 → 2026-04-24" + Gallery clear-all-three-on-× + URL round-trip) + F4 tag autocomplete (BE: new `GET /api/tags` flat-path endpoint — Option γ compromise vs Bro's Q-32.C tags-table-assumption spec — with `json_each(COALESCE(assets.tags, '[]'))` + `LIKE ? || '%' COLLATE NOCASE` + companion DISTINCT COUNT query + `listTags({q, limit})` repo method + BANNED_KEYS DTO audit add; FE: `useAssetTags` 300ms-debounced hook + pure `buildAssetTagsPath` + new `TagAutocompleteDropdown` component with prefix-match bolding + combobox state in FilterBarTagsSection with ArrowDown/Up-wrap + Enter-commits-highlighted-else-typed + Escape-close + mousedown-select + already-selected filter-out) + F5 PromptLab standalone TopNav entry (link between Gallery/Profiles + Q-32.D empty-state copy "Select an asset from the Gallery to start editing." + "Go to Gallery →" CTA + "Or open an asset from any batch and click [Edit & replay]." secondary) + F6 close (§G + this entry + HANDOFF-SESSION33). 6 verdicts locked pre-code, 3 reversed mid-stream: Q-32.B retry-with-backoff reversed after singleFork alone failed 5-run verification (evidence-driven override — intra-file async-sync race needs Node's native maxRetries, which IS the designed fix not a mask); Q-F2.2 /export-only → /export + /import clean sweep (grep proved /import symmetric dead); Q-32.C tags+asset_tags-tables spec → Option γ flat path + JSON-column response shape (audit proved §C1 trigger not tripped). Regression 700 pass / 10 skipped / 1 todo / 711 total (+15 net: 4 F3 schema + 4 F3 builder + 3 F3 URL + 4 F4-FE path). Preview MCP smoke verified all 3 UI surfaces end-to-end (F3 custom range + chip + URL sync + validation; F4-FE combobox keyboard + mouse + Escape + filter-out; F5 TopNav link + empty state + CTA). Live smoke skipped per Q-32.E ($0.92 preserved, no provider-surface changes in F1-F5). 5 commits: fix(tests) F1 + refactor(api) F2 + feat(api) F4-BE + feat(gallery,ui) F3+F4-FE+F5 + docs F6. v1 feature-complete + polish-complete + ship-ready; Session #33 = post-ship support (bug-fix on real dogfood, NOT new feature work).
Historical context for Session #31: Phase 5 closed with Step 6 (AppProfileSchema v2 migration + preserve-edits-on-409 UI). Historical step-4 prose retained: Session #30 landed Step 4 (frontend Profile CMS): `Profiles` list page (5-column table — logo+initials fallback / name+tagline / category / updatedAt relative / row actions edit-duplicate-export-delete); `ProfileEdit` dedicated editor page (create + edit modes, unsaved-changes guard via `navigator.registerGuard` + `beforeunload`, 5 sections — Identity + Assets + Visual + Positioning + Context); shared `StringListEditor` chip input with pure `normalizeItem` + `tryAddItem` + `shouldCommitOnKey` helpers for 6 schema consumers; `DeleteProfileDialog` state machine with PROFILE_HAS_ASSETS branch + View-in-Gallery CTA; client-side export via `{schemaVersion, profile, notes}` envelope + `${slugify(name)}.profile.json` download; clone-to-draft (`cloneProfileToDraft` clears assets — profile-asset FK is per-profile); `apiPut` added to API client; `profile-hooks` adds `useProfilesList`, `useProfileDetail`, mutations with 409-aware error describer. 15 new client files + 4 modified + 31 new unit tests. F1 simplified — 409 VERSION_CONFLICT preserve-edits flow deferred to v2 migration session (unreachable under `version: z.literal(1)`). 670/683 regression pass (2 remaining = known parallel ENOTEMPTY race, not introduced). Session #30 landed Step 4 (frontend — expanded Gallery filter UI): `AssetFilterChips` summary row (specificity-order chips, click-body scroll + flash + focus, × clear, Clear-all when ≥ 2); `AssetFilterBar` rewritten around a single `onChange(patch)` with 8 anchor sections (profile dropdown, workflow colored chips, batch input, tags section with free-text Enter/comma/Tab + OR/AND toggle, date preset 4-radio, replayClass 3-checkbox, provider + model multi-select with cascade toast); `GalleryEmptyState` empty-filter result block; Gallery page owns one `AssetListFilter` + bidirectional URL sync (`history.replaceState` on change, `AssetListFilterSchema.safeParse` on mount). Mid-session scope delta authorized by bro: backend schema helper `csvArrayPreserveEmpty` + query-builder `1 = 0` clause so `replayClasses === []` round-trips as "match-none" (Q-29.E option b — distinct from absent). 3 new client components + 2 new utils + 1 expanded bar + 1 expanded hooks module + 1 Gallery rewrite + 2 tiny test files (14 URL round-trip + 7 date-presets) + 8 appended schema / query-builder tests. 641 pass / 10 skipped / 1 todo / 652 total regression (+29 net vs Session #28a). Phase 4 remains closed; Phase 5 Step 4 (Profile CMS) targets Session #30; Step 6 (v2 schema migration) pending.)
Last updated: 2026-04-24 (Session #32 — Phase 6 single-session ship close-out — v1 READY declared. Scope: F1 (CF#27 parallel ENOTEMPTY race fix — forks singleFork pool on 5 chartlens-cleanup integration tests + `rmSync({recursive, force, maxRetries: 5, retryDelay: 200})` intra-file retry budget; evidence-driven Q-32.B pushback reversal after singleFork-alone 5-run verification failed 4/5 with 1-6 flakes per run; final fix 5/5 consecutive clean — 678 pass / 10 skipped / 1 todo / 689 total) + F2 (clean-sweep deletion of `/api/profiles/:id/export` + `/api/profiles/import` — Option B expansion from handoff §F2's /export-only after grep proved /import symmetric dead; handoff §21 "Profile import UI" already deferred to v2+ roadmap; commit diff +8/-90 LOC; 6 files modified — `profiles.ts` routes + `profiles.body.ts` schemas + `profile-hooks.ts` header comment + `app-profile.ts` schema comment + 2 test files; 5 tests removed — 2 export + 3 import — + 1 DTO audit entry) + F4-BE (new `src/server/routes/tags.ts` 32 LOC + `src/server/asset-store/tags-query.ts` 30 LOC with buildTagsQuery + buildTagsCountQuery using `json_each(COALESCE(assets.tags, '[]'))` + `LIKE ? || '%' COLLATE NOCASE` + `GROUP BY json_each.value ORDER BY count DESC, tag ASC LIMIT ?`; `asset-repo.listTags({q, limit})` method; `app.ts` route wired at `/api/tags`; TagsQuerySchema zod — q max 100 chars + limit int 1-50 default 10; `dto-no-paths-full.test.ts` audit list adds /api/tags for path-leak parity; 9 integration cases + 3 SQL shape unit cases; path chosen flat per Q-32.C future-proofing, response shape `{tags: [{tag, count}], total}` matches current JSON-column storage — Option γ compromise vs Bro's original Q-32.C spec (which assumed a tags table + asset_tags JOIN that §C1 still hasn't triggered); response grows additively — {id?, color?, createdAt?} — when §C1 migration ships) + F3 (Gallery custom date picker — schema extends `asset-list-filter.ts` with `dateFrom` + `dateTo` ISO-date regex fields passed through transform + emptyAssetListFilter; `asset-list-query.ts` adds `localDayStart` + `localDayEnd` helpers and custom-range override branch — when either bound set, skip `datePresetBoundary(…)` entirely and emit `created_at >= ?` / `created_at <= ?` clauses; client `hooks.ts` AssetsFilter + buildAssetsQueryString emit URL-encoded `dateFrom=` + `dateTo=` params; `FilterBarEnumSections.tsx` DateSection extended with "Custom range..."/"Hide custom range" expander + 2 `<input type="date">` + Clear range button + dateFrom > dateTo `role="alert"` inline validation; toggle override — picking a preset clears dateFrom/dateTo, setting either custom bound clears preset; `AssetFilterChips.tsx` datePreset resolver checks dateFrom/dateTo first and renders "2026-04-20 → 2026-04-24" format; `Gallery.tsx` `clearDimension("datePreset")` special-cases to reset all 3 date fields on chip × to prevent preset leak after custom range used; 5 design Qs resolved — A3 toggle override / B1 extend inline / C1 client-side validation / D1 ISO chip format / E3 local day boundary; 4 schema cases + 4 builder cases + 3 URL round-trip cases added) + F4-FE (tag autocomplete combobox — `useAssetTags(q, limit, 300ms)` hook + pure `buildAssetTagsPath` URL builder wraps existing useFetch; new `TagAutocompleteDropdown.tsx` 78 LOC pure-presentation listbox with prefix-match bold via `<strong>` + count badge + `onMouseDown` to avoid input blur before selection; `FilterBarTagsSection.tsx` extended with `focused` + `highlightIdx` state + combobox aria attrs + keyboard handlers — ArrowDown/Up cycle with wrap / Enter commits highlighted else typed draft / Tab same / Escape closes + already-selected tags filtered out client-side; 4 `buildAssetTagsPath` unit cases) + F5 (PromptLab standalone TopNav entry — `TopNav.tsx` inserts "PromptLab" NavLink between Gallery/Profiles mapped to `prompt-lab` Page target; `PromptLab.tsx` empty-state branch copy refined per Q-32.D: "Select an asset from the Gallery to start editing." primary + "Go to Gallery →" CTA + "Or open an asset from any batch and click [Edit & replay]." secondary; inline asset-picker rejected — duplicates Gallery functionality + teaches wrong mental model) + F6 (PHASE-STATUS §S32 + DECISIONS §G + HANDOFF-SESSION33). Preview MCP smoke end-to-end verified on all 3 UI surfaces: F3 custom range inputs render, URL sync writes dateFrom=+dateTo=, chip shows "Date: 2026-04-20 → 2026-04-24", preset-click clears custom, custom-edit clears preset, chip × clears all 3 date fields, dateFrom>dateTo inline validation error renders; F4-FE focus opens dropdown (fetches `/api/tags?limit=10` → "memory:105"), ArrowDown highlights aria-selected=true, Enter commits chip, mousedown commits, Escape closes dropdown, already-selected tags filtered out, typed "zzz" hides dropdown for no match; F5 TopNav "PromptLab" link renders active-pill state, empty state shows exact Q-32.D copy + CTA + secondary guidance, CTA navigates to Gallery. 5 commits: `fix(tests): F1 CF#27 flake` (vitest.config.ts poolMatchGlobs + 5 afterEach rmSync retry-budget edits) + `refactor(api): F2 remove unused /profiles/:id/export + /profiles/import endpoints` (6 files + 8 insertions / 90 deletions) + `feat(api): F4 GET /api/tags distinct tag autocomplete endpoint` (8 files + 274 insertions) + `feat(gallery,ui): F3 custom date picker + F4-FE tag autocomplete + F5 PromptLab entry` (16 files + 498 insertions / 26 deletions) + `docs: Session #32 close` (this). 700 pass / 10 skipped / 1 todo / 711 total regression (+15 net vs S#31: 12 F4-BE tests + 4 schema + 4 builder + 3 URL + 4 path + 1 DTO audit - 6 F2 removed = +22 gross, -6 removed = +16 net... actually +15 per recount). Live smoke SKIPPED per Q-32.E (no provider-surface changes; $0.92 preserved for next provider-touching session). Out-of-scope for v2+ per §G.7: server-side async flush await (F1 retrospective); §C1 asset_tags JOIN migration (trigger-gated); component + hook tests (jsdom CF#5); live HTTP capability test (CF#1); tree view (#10); side-by-side diff panel (#11); cursor-based pagination (#14); Profile import UI (#21) — redesign fresh when a real user surfaces the need; 3-way merge (#22); bulk profile ops (#23); profile list search (#24); read-only profile view (#25). Session #33 = post-ship support — bug-fix flow on real dogfood findings, NOT new feature work.)
Last updated-prior: 2026-04-24 (Session #31 — Phase 5 Step 6 ship (AppProfileSchema v2 migration via z.union + .transform pattern, DECISIONS §F). 4 Qs locked pre-code with one bro pushback that reversed direction pre-code: Q-31.A Option A full ship this session; Q-31.C PUSHBACK → migration-on-read via z.union + transform (rejected in-place-on-write — fragmented state, additive-only assumption, masks bugs, v1-consumer rot); Q-31.C.1 v2a minimal — V1 version: z.literal(1) + V2 version: z.number().int().min(1) + identity transform (no shape change); Q-31.D preserve-edits-on-409 UI: non-dismissible banner + state machine (saved / conflict / overwrite-save / discard-with-confirm) + native window.confirm for discard + Overwrite & Save label swap. Schema: `src/core/schemas/app-profile.ts` rewrite — `AppProfileBodyFields` (exported, version-agnostic) → `V1Schema` + `V2Schema` extends → `z.union([V1,V2]).transform(migrateToV2)` → `AppProfile = z.output<...>` (v2 shape). Server: `src/server/routes/profiles.ts` PUT bumps `next.version = existing.version + 1` + 409 body augmented with `code: "VERSION_CONFLICT"` + `details: { currentVersion, expectedVersion }` alongside legacy flat fields (DECISIONS §F.3.1); `profile-assets.ts` 409 body mirror + comment note that upload doesn't bump (F.3). Saver unchanged (comment updated — storage-neutral per §F.3). Body schemas: `profiles.body.ts` uses `AppProfileBodyFields.omit(...)` (can't .omit on ZodEffects). Migration: `scripts/migrate-profiles-v1-to-v2.ts` (idempotent, scan→validate→re-write, exit 1 on parse fail) + `migrate:profiles-v1-to-v2` npm script. Verified against 3 seeded profiles — 3 "migrated" (bytes re-normalized) first run → 3 noop second run. Client: `profile-hooks` adds `parseVersionConflict(err)` reading `ApiError.details` + describeSaveFailure 409 reworded ("Remote updated — reloaded latest, your edits kept."); `ProfileConflictBanner` new 48-LOC component (role=alert, Discard button with native confirm); `ProfileEdit.tsx` conflict state (`{ remoteVersion: number } | null`) + handleSave 409 branch that apiGets latest + updates initial + preserves form + sets conflict + shows toast + swaps Save label to "Overwrite & Save" + handleDiscardEdits reverts form to initial. Files: 3 new (ProfileConflictBanner 48 / migrate-profiles-v1-to-v2.ts 81 / app-profile-migration.test.ts 99) + 9 modified (app-profile.ts schema refactor 110 LOC / profiles.ts PUT bump + 409 augment / profile-assets.ts 409 augment + comment / saver.ts comment only / profiles.body.ts AppProfileBodyFields import / profile-hooks.ts parseVersionConflict + reword / ProfileEdit.tsx conflict state machine 291 LOC / profiles-crud.test.ts adjust v2 bump + real-409 flow / profiles-routes.test.ts adjust v2 bump + augmented 409 assertions) + 1 test comment-only (app-profile.test.ts Rule 14 wording) + package.json +1 script. 676/689 regression pass (+6 net vs Session #30 — 5 migration unit + 1 preserve-edits integration; 2 failed = known parallel ENOTEMPTY race CF#27 in replay-route.test.ts, isolated-retry 10/10 pass, not introduced). Preview MCP smoke end-to-end: clean load → local edit → external PUT (simulates tab A) → form Save → 409 → banner appears + Overwrite & Save label + Discard CTA + tagline preserved → click Overwrite & Save → bumps version to 3 + banner clears + Save reverts → second edit + second external PUT → form Save → second 409 → click Discard → native confirm fired with correct copy → revert to remote + banner clears + Save disabled. Test profile `zz-mcp-smoke` created + cleaned up 204 at smoke close.)
...
Last updated-prior-prior: 2026-04-24 (Session #29 — Phase 5 Step 3b ship (frontend + a narrow backend schema relax for Q-29.E match-none semantics). 6 Qs locked pre-code with one bro pushback: Q-29.A chips in specificity order (batch → profile → workflow → tags → replayClass → provider → model → date) rather than card-order; Q-29.B tags free-text with Enter/comma/Tab delimiters + trim/lowercase/whitespace-collapse normalization + dedup toast + backspace-on-empty removes last; Q-29.C URL CSV with per-value `encodeURIComponent` + un-escaped comma separator (keeps `?tags=sunset,neon` readable in the address bar); Q-29.D provider → model cascade clears only invalid models, toast once per transition; Q-29.E (option b) replayClasses `[]` preserved on the wire + SQL builder emits `1 = 0` so "0 of 3 checkboxes selected" = return zero rows (new schema helper `csvArrayPreserveEmpty` leaves plural non-replay fields unchanged); Q-29.F Step 3b clean ship + CMS deferred to Session #30 with a 30–60min dogfood window between sessions. Scope notes: (1) tag filter = LIKE scan on the JSON `tags` column (honors DECISIONS §C1 post-v1 deferral); (2) path layout — new schema at `src/core/schemas/asset-list-filter.ts`, SQL builder alongside asset-repo at `src/server/asset-store/asset-list-query.ts`, no new `src/server/assets/` or `src/client/hooks/` dirs; (3) skip totalCount v1 (response shape stays `{ assets, limit, offset }`). Commit chain: (1) `feat(gallery)` ships 2 new files + 5 modified (asset-repo drops 20 LOC of inline WHERE building; assets.body.ts reduced to re-export shim; assets.ts route drops custom `coerceQuery` in favor of schema-strict `.safeParse` with CSV preprocess). Wire contract: plural CSV params (`profileIds=a,b,c`) preferred; legacy singular (`profileId=a`) still parses and merges into plural array via `.transform()`. Cursor-based pagination explicitly deferred to Session #30+ with inline TODO. 39 new tests: 12 in `asset-list-filter-schema.test.ts` (strict-allowlist + CSV preprocess + enum bounds + limit clamp + legacy merge), 19 in `asset-list-query-builder.test.ts` (WHERE composition + tag OR|AND + LIKE escape + datePreset boundaries + param ordering), 8 appended to `assets-routes.test.ts` (HTTP end-to-end on profileIds / replayClasses / providerIds+modelIds / tags OR|AND / datePreset=7d / strict-allowlist 400 / legacy singular backward-compat). 1 existing test updated in `asset-store.test.ts` (`profileId` → `profileIds: [X]` for the new interface shape — non-API-breaking, only the repo filter param renamed). 612/623 regression pass — clean full run with no flake observed. `test:live:smoke-all` NOT run (no provider surface change). Manual UI smoke remains deferred (Step 2 + Step 5b batch) since 28a is backend-only; 28b Session #29 + smoke can co-locate or run as a separate Chrome MCP session at office.)

## Phase 6 Summary (CLOSED — v1 READY)

| Feature | Title | Status |
|---|---|---|
| F1 | CF#27 parallel ENOTEMPTY race fix | ✅ Session #32 — vitest.config.ts poolMatchGlobs routing 5 chartlens-cleanup integration tests (edit-and-run, replay-route, workflows-full, workflows-routes, workflows-cancel) to a forks pool with `singleFork: true` + `rmSync({recursive, force, maxRetries: 5, retryDelay: 200})` 1s retry budget on each afterEach. Evidence-driven Q-32.B pushback reversal: singleFork alone failed 5-run verification (1/8 runs had 1 cold-start failure); the race isn't purely cross-file — server asset-store continues flushing writes after HTTP response returns, afterEach rmSync fires during flush, ENOTEMPTY surfaces. Native maxRetries IS the correct fix for OS-level Windows transient FS races. 5/5 consecutive clean regression runs verified. |
| F2 | Clean-sweep dead-code deletion (/export + /import) | ✅ Session #32 — deleted both route handlers in `src/server/routes/profiles.ts`, dropped ProfileImportBodySchema/Body exports in `profiles.body.ts`, removed 2 integration describe blocks (5 tests total) + /export entry in DTO audit list. Q-F2.2 scope expansion justified via grep: /import also dead (zero src/ consumers, handoff §21 already defers "Profile import UI" to v2+). Commit +8/-90 LOC. |
| F3 | Gallery custom date picker | ✅ Session #32 — schema `asset-list-filter.ts` adds `dateFrom` + `dateTo` ISO-date fields; SQL builder `asset-list-query.ts` gets `localDayStart`/`localDayEnd` helpers + custom-range override branch; client `hooks.ts` AssetsFilter + buildAssetsQueryString emit URL-encoded params; DateSection extended inline (B1) with expander + 2 date inputs + dateFrom>dateTo inline validation; AssetFilterChips renders "2026-04-20 → 2026-04-24"; Gallery clearDimension("datePreset") resets all 3 fields. 5 Qs resolved: A3 toggle override / B1 extend inline / C1 client-side validation / D1 ISO chip format / E3 local day boundary. Preview MCP smoke: custom range inputs render, URL sync, chip format, preset/custom mutual clear, chip × clears all 3, validation error. |
| F4 | Tag autocomplete (BE + FE) — CF#19 | ✅ Session #32 — BE: new `tags.ts` route + `tags-query.ts` SQL builders via `json_each(COALESCE(assets.tags, '[]'))` + LIKE NOCASE prefix + DISTINCT COUNT companion query + `asset-repo.listTags({q, limit})` + `/api/tags` wired; FE: `useAssetTags` debounced hook (300ms) + pure `buildAssetTagsPath` + new `TagAutocompleteDropdown` 78-LOC component + FilterBarTagsSection combobox with ArrowDown/Up-wrap + Enter-commits-highlighted-else-typed + Escape-close + mousedown-select + already-selected filter-out. 9 integration + 3 SQL shape + 4 path-builder unit tests. Path flat per Q-32.C future-proofing, response shape `{tags: [{tag, count}], total}` matches JSON-column storage — Option γ compromise vs Bro's original spec (audit proved §C1 asset_tags JOIN trigger not tripped). Preview MCP smoke: focus opens dropdown, fetches tag+count, ArrowDown highlights aria-selected=true, Enter commits, mousedown commits, Escape closes, already-selected filtered out. |
| F5 | PromptLab standalone TopNav entry — CF#12 | ✅ Session #32 — `TopNav.tsx` inserts "PromptLab" NavLink between Gallery/Profiles; `PromptLab.tsx` empty-state branch copy refined per Q-32.D: "Select an asset from the Gallery to start editing." + "Go to Gallery →" CTA + "Or open an asset from any batch and click [Edit & replay]." Inline asset-picker rejected — duplicates Gallery functionality + teaches wrong mental model. Preview MCP smoke: TopNav link active-pill state, copy + CTA exact, CTA navigates to Gallery. |
| F6 | Phase 6 close — docs + handoff | ✅ Session #32 — this PHASE-STATUS §S32 entry + DECISIONS §G (Session #32 rationale, pushback evolution stories, §G.7 out-of-scope list for v2+) + HANDOFF-SESSION33.md (post-ship support flow, bug-fix template, dogfood-findings placeholder, v2+ roadmap seed). Live smoke `test:live:smoke-all` skipped per Q-32.E ($0.92 preserved, no provider-surface changes in F1-F5). 5 commits per Q-32.F + Q-F2.push: fix(tests) F1 pushed solo + refactor(api) F2 + feat(api) F4-BE pushed as 2a+2b batch + feat(gallery,ui) F3+F4-FE+F5 UI bundle + docs F6. |

## Phase 5 Summary (CLOSED)
...
| 6 | AppProfileSchema v2 migration + preserve-edits-on-409 UI | ✅ Session #31 — 3 new files (`src/client/components/profile-editor/ProfileConflictBanner.tsx` 48 LOC / `scripts/migrate-profiles-v1-to-v2.ts` 81 LOC / `tests/unit/app-profile-migration.test.ts` 99 LOC — 5 cases) + 9 modified (`src/core/schemas/app-profile.ts` 110 LOC — AppProfileBodyFields exported + V1Schema/V2Schema + identity `migrateToV2` + `z.union([V1,V2]).transform(migrateToV2)`; `src/server/routes/profiles.ts` PUT bumps `next.version = existing.version + 1` + 409 body augmented with `code` + `details: {currentVersion, expectedVersion}`; `src/server/routes/profile-assets.ts` 409 body mirror; `src/server/profile-repo/saver.ts` comment only (storage-neutral per DECISIONS §F.3); `src/server/routes/profiles.body.ts` uses `AppProfileBodyFields.omit(...)` since `.omit()` unavailable on ZodEffects; `src/client/api/profile-hooks.ts` adds `parseVersionConflict(err)` helper reading `ApiError.details` + describeSaveFailure 409 reworded to the Q-31.D toast copy; `src/client/pages/ProfileEdit.tsx` 291 LOC — conflict state machine (`{ remoteVersion: number } \| null`) + handleSave 409 branch (apiGet latest, preserve form, set conflict, swap Save label to "Overwrite & Save") + handleDiscardEdits (native confirm → revert to refetched initial); `tests/integration/profiles-crud.test.ts` adjust v2 bump + real-409 flow ("two tabs, loser refetches + Overwrite-Saves"); `tests/integration/profiles-routes.test.ts` adjust v2 bump + augmented 409 assertions) + 1 test comment wording (app-profile.test.ts) + package.json +1 script (`migrate:profiles-v1-to-v2`). Preview MCP smoke verified end-to-end (banner + Overwrite + Discard + confirm copy). 676/689 regression pass (+6 net vs Session #30 — 5 migration unit + 1 preserve-edits integration). 2 remaining = known parallel ENOTEMPTY race CF#27, isolated-retry 10/10 pass. |
